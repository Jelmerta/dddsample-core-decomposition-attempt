...
public class ItineraryCandidateDTOAssembler {
  ...
  protected LegDTO toLegDTO(final Leg leg) {
    ...
    final String from = LocationClient.sampleLocationsGetLocation(leg.loadLocation().idString()).getUnLocode().getUnlocode();
    final String to = LocationClient.sampleLocationsGetLocation(leg.unloadLocation().idString()).getUnLocode().getUnlocode();
    return new LegDTO(voyageNumber.idString(), from, to, leg.loadTime(), leg.unloadTime());
  }
...
//                           final LocationRepository locationRepository) { // TODO???
    ...
    for (LegDTO legDTO : routeCandidateDTO.getLegs()) {
      ...
      // TODO These should be unlocodes
      final LocationId from = new LocationId(LocationClient.sampleLocationsGetAll().stream().filter(l -> l.getUnLocode().getUnlocode().equals(legDTO.getFrom())).findFirst().orElseThrow(IllegalArgumentException::new).getName());
      final LocationId to = new LocationId(LocationClient.sampleLocationsGetAll().stream().filter(l -> l.getUnLocode().getUnlocode().equals(legDTO.getTo())).findFirst().orElseThrow(IllegalArgumentException::new).getName());
