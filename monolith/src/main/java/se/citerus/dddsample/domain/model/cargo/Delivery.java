...
public class Delivery implements ValueObject<Delivery> {
    ...
    private LocationId lastKnownLocation;
     ...
     */
    public LocationId lastKnownLocation() {
        // TODO Not very clean maybe... But the id is currently same as name...
        return DomainObjectUtils.nullSafe(lastKnownLocation, new LocationId(LocationClient.sampleLocationsGetLocation("UNKNOWN").getName()));
    }
    ...
    private LocationId calculateLastKnownLocation() {
        if (lastEvent != null) {
        ...
        switch (lastEvent.type()) {
            ...
            case LOAD:
                for (Leg leg : itinerary.legs()) {
                    if (LocationClient.locationSameIdentityAs(leg.loadLocation().idString(), lastEvent.location().idString())) {
            ...
            case UNLOAD:
                for (Iterator<Leg> it = itinerary.legs().iterator(); it.hasNext(); ) {
                    ...
                    if (LocationClient.locationSameIdentityAs(leg.unloadLocation().idString(), lastEvent.location().idString())) {
                        if (it.hasNext()) {
    ...
    private boolean calculateUnloadedAtDestination(RouteSpecification routeSpecification) {
        return lastEvent != null &&
                ...
                LocationClient.locationSameIdentityAs(routeSpecification.destination().idString(), lastEvent.location().idString());
    }
